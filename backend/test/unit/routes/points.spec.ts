...
describe('Controller: Points', () => {
    ...
    describe('show()', () => {
        it('Should return a point and its items passing an point id', async () => {
            ...
            dbMock.expects('where').once().withArgs('point_item.point_id', pointId)
            dbMock.expects('select').once().withArgs('items.title').resolves([fakeItem])
            ...
            expect(response.json).toBeCalledWith(
                expect.objectContaining({
                    ...
                    items: expect.arrayContaining([fakeItem])
                }))
        })
    })
